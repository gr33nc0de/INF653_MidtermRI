...
// Create Database object and connect to the database
...
if (!empty($data->quote) && isset($data->author_id) && isset($data->category_id)) {
    ...
    if (!$authorExists) {
        ...
        return; // Stop execution if author_id not found
    }

    if (!$categoryExists) {
        echo json_encode(array('message' => 'category_id Not Found'));
        return; // Stop execution if category_id not found
    }
    ...
    if ($quote->create()) 
    {
        // If the quote was successfully created
        $newQuoteData = array(
            'id' => $db->lastInsertId(),
            'quote' => $data->quote,
